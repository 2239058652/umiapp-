<template>
	...
	<!-- 	<scroll-view
		...
		> -->
			<!-- #endif -->
		<!-- 	#ifdef APP-NVUE || MP-WEIXIN || H5 || APP-VUE || MP-QQ -->
			<!-- 非以上平台，只能使用普通js实现 -->
			<scroll-view
				class="u-scroll-list__scroll-view"
				scroll-x
...
export default {
	...
	computed: {
		...
		barStyle() {
			...
			const scrollLeft = this.scrollInfo.scrollLeft,
				scrollWidth = this.scrollInfo.scrollWidth,//426
				barAllMoveWidth = this.indicatorWidth - this.indicatorBarWidth
			const x = scrollLeft / this.indicatorWidth * barAllMoveWidth//50
			style.transform = `translateX(${ x }px)`   //356.25
			// #endif
	...
	methods: {
      ...
      this.scrollInfo.scrollLeft <= 10 ? this.scrollInfo.scrollLeft = 0 : ""
      this.scrollInfo.scrollLeft >= 50 ? this.scrollInfo.scrollLeft = 50 : ""
		},
		scrolltoupperHandler() { //滚动到左边触发
			this.scrollEvent('left')
			this.scrollInfo.scrollLeft = 0
		},
		scrolltolowerHandler() { //滚动到右边触发
