				...
				>
					...
					>
						...
						>
							...
							>{{ item[keyName]||item.floorItem }}</text>
							<u-badge
	...
	export default {
		...
		computed: {
			textStyle(a) {
				return i => {
					const style = {}
					// 取当期是否激活的样式
					const customeStyle = i == this.innerCurrent ? uni.$u.addStyle(this.activeStyle) : uni.$u
		...
		methods: {
			setLineLeft() {
				...
				this.lineOffsetLeft = lineOffsetLeft + (tabItem.rect.width - lineWidth+24) / 2
				// #ifdef APP-NVUE
				// 第一次移动滑块，无需过渡时间
