...
<script>
	...
	export default {
		...
		methods: {
			...
			bian(data){
				this.children.map(child => {
					child.isChecked=data
				})
			},
			// 将其他的checkbox设置为未选中的状态
			unCheckedOther(child) {
				const values=[].concat([child.name],[child.isChecked],[child.price],[child.ids])
				
				// this.children.map(child => {
				// 	// 将被选中的checkbox，放到数组中返回
				// 	// if (child.isChecked) {
				// 		values.push([child.name,child.isChecked,child.price,child.ids])
				// 	// }
				// })
				// 发出事件
