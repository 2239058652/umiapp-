	...
	<view class="pageheader" :style="bj">
		<view v-if="ishide">
			...
			<view :style="{marginTop:distance}">
				<text class="store" @click="location">{{bh_name}}</text>
...
<script>
	...
	export default {
		...
		created() {
			// 如果没有在百货页面选择门店的话 默认暂时距离最近的(第一个)
			if(!uni.getStorageSync('bh_name')&&!uni.getStorageSync('bh_num')){
			uni.setStorageSync('bh_name',this.bh_list_mendian[0].name)
			uni.setStorageSync('bh_num',this.bh_list_mendian[0].storeSn)
			this.$store.commit("setStorebh_name", this.bh_list_mendian[0].name)
			}else{	
				// console.log('百货页面已经选择了')
			}
		},
		data() {
			return {
