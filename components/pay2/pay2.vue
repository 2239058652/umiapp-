...
<script>
	...
	export default {
		...
		methods: {
			...
			// 卡支付
			cardPay() {
				console.log('点击卡支付', this.cardNoList);
							...
							uni.requestSubscribeMessage({
								...
								complete(res) {
									if (that.text == "微信支付" && that.choice.length != 1) {
										console.log('使用微信支付',that.price,that.a);
										...
										uni.$u.http.post('/card-littlemall/orderManage/placePay', {
											...
											if (timeStamp && nonceStr && packages && signType &&
												paySign) {
												uni.getSetting({
