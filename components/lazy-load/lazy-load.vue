<template>
    <view class="u-lazy u-wrap" :style="{
      opacity: Number(opacity),//透明度
      transition: `opacity ${time / 1000}s ease-in-out`, borderRadius}">
			<!-- 加载成功 -->
      <image :style="{ borderRadius}" 
        ...
        :mode="imgMode" 
        @load="imgLoaded" @error="loadError" @tap="clickImg">
      </image>
			<!-- 加载失败 -->
      <image :style="{ borderRadius}" 
...
export default {
  ...
  props: {
		layoutType:{
			type: String,
			 default: 'elasticLayout1'
		},
    index: {//自定义值，在事件触发时回调用于区分是那个图片
      type: [Number, String]
    },
    // 要显示的图片路径
    image: {
  ...
  data() {
    return {
      isShow: true, //默认显示占位图
      opacity: 1, //默认不透明
      time: this.duration, // 淡入淡出动画的过渡时间
  ...
  computed: {
    ...
    getThreshold() {
      ...
      let thresholdPx = uni.upx2px(Math.abs(this.threshold));//去绝对值后转化为px
  ...
  mounted() {
    ...
    setTimeout(() => {
      ...
      contentObserver.relativeToViewport({ //选择一个显示区域作为参照
          bottom: this.getThreshold
        }).observe('.u-lazy', res => {  //选择目标节点为“u-lazy”
          if (res.intersectionRatio > 0) { //intersectionRatio相交比列大于0的话表示两者有了交集，等于1表示完全相交
            // 懒加载状态改变
            this.isShow = true;
...
@mixin lazy-load-img ($width,$height){
...
//首页分类布局
.elasticLayout1 { @include lazy-load-img(330rpx,330rpx)}
.elasticLayout2 { 
	@include lazy-load-img(100%,348rpx);
	border-radius: 30rpx 30rpx 0 0 !important;
}
.elasticLayout3 { 
	@include lazy-load-img(100%,234rpx);
	border-radius: 30rpx 30rpx 0 0 !important;
}
.elasticLayout4 { @include lazy-load-img(260rpx,260rpx)}
//积分商城
.pointLayout1{ 
	@include lazy-load-img(330rpx,330rpx)
}
.pointLayout2{
	@include lazy-load-img(100%,324rpx)
}
.pointLayout3{
	@include lazy-load-img(100%,234rpx)
}
