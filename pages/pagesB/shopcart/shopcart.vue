	...
	<view class="shoppingCart ">
		<view class="pageheader" :style="bj">
			...
			<view :style="{marginTop:distance}">
				<text class="aj aj-xiangzuojiantou cart_mendian" style="color: #fff;margin-left: 20rpx;" @click="back">百货</text>
				<text class="text" @click="changeI">{{isText ?'编辑':'完成'}}</text>
			</view>
			<!-- 搜索 -->
		...
		<!-- 骨架屏 -->
		<view class="body " v-show="judge" :style="{paddingTop:topHeight+'px'}">
			<!-- 轮播 -->
			...
			<uni-swipe-action>
				<view class="StoreGoods" v-for="(allitem,index) in list" :key="index">
					<view class="storeName">
						...
						<view class="zgdj" v-if="allitem.isZg==true">专柜</view>
						<view class="aj aj-jinrujiantou" @click="gozgStore(allitem.cartItems[0].shopId,allitem.isZg)">
							{{ allitem.shopName || allitem.zgName}}
						</view>
					</view>
					<uni-swipe-action-item :right-options="options1" v-for="(item,gindex) in allitem.cartItems"
						:key="gindex" @click="del(item.id)" autoClose="true">
						<view class="commoditys">
							<view :class="item.selected?'active goodscircle':'goodscircle'"
								@click="chooseGoodsSelect(index,gindex)">
								<u-icon name="checkbox-mark" color='#fff' size="40"></u-icon>
							</view>
							<image mode="scaleToFill" lazy-load :src="item.pic"
								@click="goDetails(allitem.isZg,item.goodsId,allitem.cartItems[0].shopId)" />
							<!-- 商品简介 -->
							<view class="navTab">
								<view>{{item.barcode}}</view>
								<view>{{ item.name }}</view>
								<view v-show="false">
									商品规格:{{item.goodsSpecificationDetailList[0].name}}<text
										class="aj aj-a-9youjiankuohao"></text>
									换促销<text class="aj aj-a-9youjiankuohao"></text>
								</view>
								<view>
									<text>￥{{item.yprice}}<text class="miaosha" v-if="item.type == 1">秒杀</text></text>
									<u-number-box ref="numberBox" v-model="item.quantity" integer="false"
										disabledInput="true" @overlimit="overlimit" :disablePlus="disablePlus"
										@blur="blur(item)">
		...
		<!-- 当用户没有登录  -->
		<template v-if="!judge">
			<u-empty text="登录后同步查看订单信息" mode="car" marginTop="45% 0 0 0" textSize="30" iconSize="160"
				iconColor="#bcbcbc"></u-empty>
			<u-button type="success" size="40" @click="jump('/pages/pagesA/denglu/denglu')">登录</u-button>
		</template>
		<view class="low u-skeleton-fillet" v-else>
			<view :class="isCheckAll?'active circle':'circle'" @click="chooseCheckAll">
				<u-icon name="checkbox-mark" color='#fff' size="40"></u-icon>
			</view>
			全选
			<!-- 使用v-show失效 -->
			<view class="view" v-if="isText">
				<text>合计</text><text>￥</text><text>{{totalPrice.toFixed(2)}}</text><text
					@click="button">去结算({{num}})</text>
			</view>
			<view class="view" v-else>
				<text @click="button('detele')">删除</text>
			</view>
		</view>
		<view class="kong" v-show="judge&&listLength==0">
			<u-empty mode="car" text="你还没有添加宝贝!" iconColor="#bcbcbc" marginTop="30% 0 0 0" textSize="30" iconSize="160">
			</u-empty>
			<view @click="jump('/pages/tabbar/index/index')">去逛逛</view>
		</view>
		<!--  专柜和普通不能一起结算-->
		<u-popup :show="settlementShow" @close="settlementShow=false" round="30" closeIconPos="top-right"
			closeable="false">
			<view class="settlement">
				...
				<view>
					...
					<scroll-view scroll-x="true" @click="scroll">
						<image v-for="(img,ind) in mdpop[0].imglist" :key="ind" :src="img" mode=""></image>
					</scroll-view>
					<view class="view">
						<text>合计</text><text>￥</text><text>{{mdpop[0].total}}</text><text
							@click="btnSettlement(1)">去结算({{mdpop[0].num}})</text>
					</view>
				</view>
				<view>
					<view>专柜到家</view>
					<scroll-view scroll-x="true" @click="scroll">
						<image v-for="(img,ind) in zgpop[0].imglist" :key="ind" :src="img" mode=""></image>
					</scroll-view>
					<view class="view">
						<text>合计</text><text>￥</text><text>{{zgpop[0].total}}</text><text
							@click="btnSettlement(0)">去结算({{zgpop[0].num}})</text>
					</view>
				</view>
			</view>
		</u-popup>
		...
		methods: {
			...
			selectCartItem(data) {
				return new Promise((resolve) => {
					this.$request.getSelectCartItem({
			...
			goDetails(isZg, id, shopId) {
				if (isZg) {
					this.jump(`/pages/pagesA/goodsDetails/goodsDetails?id=${id}_${shopId}&dshopType=0003`)
				} else {
			...
			btnSettlement(e) {
				if (e == 0) {
					// 专柜
					this.jump(
						`/pages/pagesA/order/order?parameter=${JSON.stringify({ids:this.zgpop[0].id,totalPrice:this.zgpop[0].total,shopId})}&msprice=undefined`
					)
				} else {
					// 超市
					this.jump(
						`/pages/pagesA/order/order?parameter=${JSON.stringify({ids:this.mdpop[0].id,totalPrice:this.mdpop[0].total})}&msprice=undefined`
					)
				}
			},
			button(res) {
				...
				// 		})
				// 	} else {
				// 		if (this.zgpop[0].imglist.length != 0 && this.mdpop[0].imglist.length != 0 || this.chooseZg
				// 			.length > 1) {
				...
				// 		}
				// 		this.jump(
				// 			`/pages/pagesA/order/order?parameter=${JSON.stringify({ids:this.id,totalPrice:this.totalPrice.toFixed(2)})}&msprice=undefined`
				// 		)
				// 	}
				...
				for (let i = 0; i < this.zgGoodsList.length; i++) {
					for (let j = 0; j < this.zgGoodsList[i].cartItems.length; j++) {
						if (this.zgGoodsList[i].cartItems[j].selected) {
							zgtotal += (this.zgGoodsList[i].cartItems[j].yprice * this.zgGoodsList[i].cartItems[j]
								.quantity);
			...
			// 更改数量
			updateNum(id, quantity, item) {
				if (quantity <= 0) {
					this.isShowToast('商品数量不能小于0')
					item.quantity = 1
					quantity = 1
				}
				this.$request.getUpdateCart({
					id: id,
					quantity: quantity,
			...
			minus(item) {
				...
				if (item.quantity > 1) {
					this.$store.dispatch('judge', [item.quantity - 1, item.sn + "_" + item.shopId]).then(res => {
						if (res[0] == false) {
							this.isShowToast(`宝贝只剩最后${item.quantity}个了！`)
						} else {
							item.quantity -= 1
							this.updateNum(item.id, item.quantity, item)
							this.alltotalPrice()
						}
					})
				}
			...
			plus(item) {
				if (this.time) return
				this.$store.dispatch('judge', [item.quantity, item.sn + "_" + item.shopId]).then(res => {
					if (res[0] == false) {
