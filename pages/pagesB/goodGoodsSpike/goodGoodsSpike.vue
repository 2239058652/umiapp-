<template>
	<view class="goodsSpike">
		<uni-nav-bar @clickLeft="back" leftWidth="120px" fixed="false" statusBar="false" leftText="秒杀" left-icon="back">
		</uni-nav-bar>
		<view class="fixed" v-if="show">
			<view class="line"></view>
			<roll ref="roll" :list="times" :isshow="true" font="false" @postAjaxSeckill="postAjaxSeckill" right="0"
				left="0" both="0 20rpx"></roll>
			<view class="text">
				<text>{{spikeText}}：</text>
				<u-count-down :time="countdown" format="HH:mm:ss" autoStart @change="Change" @finish="finish">
					<view class="time">
						<view class="time__custom" v-if="countDown(timeData.days) != 0">{{countDown(timeData.days)}}
						</view>
		...
		<view class="lists" v-if="show">
			<view v-for="(item,index) in lists" :key="index" :style="item.goodsNum == 0 && ['pointer-events:none']"
				@click.stop="jump(`/pages/pagesA/goodsDetails/goodsDetails?id=${item.sn}_${shopNo}&msprice=${item.price}&ygms=${status.description}&spickId=${item.id}&xiangou=${item.goodsLimited}&xiangou2=${item.goodsMinimum}`)">
				...
				<view class="describe">
					...
					<view>秒杀价￥{{item.price}} <text>￥{{item.yprice}}</text></view>
					<view class="you" v-if="item.vipPrice!=''">
						<view>优<text>+</text></view>
						<view><text>￥</text>{{item.vipPrice}}</view>
					</view>
		...
		<view v-if="!show">
			<u-empty mode='data' icon-size="250" font-size="35" margin-top="150" textSize="34" text="暂无秒杀活动"></u-empty>
		</view>
...
<script>
	...
	export default {
		...
		onShow() {
			// uni.request({
			// 	url: 'http://192.168.1.7:8107/wpSpikeInfo/ajaxSeckill',
			// 	method: 'post',
			// 	data: {
			// 		shopNo: "04027",
			// 		spikeTimer: "2023/06/27 07:00:00",
			// 		tenantId: "39602",
			// 	},
			// 	header: {
		...
		methods: {
			queryTimer() {
				this.$request.postQuerySpikeStartTimer({}).then(res => {
					res.todayTimer.forEach((item, index) => {
						this.Status(this.timeStamp(item.spikeEndTimer), this.timeStamp(item
								.spikeStartTimer),
							res.todayTimer, item)
					})
					res.tomorrowTimer.forEach((item, index) => {
						this.Status(this.timeStamp(item.spikeEndTimer), this.timeStamp(item
								.spikeStartTimer),
							res.tomorrowTimer, item, item.spikeStartTimer.slice(0, 10))
					})
					...
					this.times.length == 0 && uni.showToast({
						title: "活动已结束",
						icon: "none",
						duration: 2000,
					});
			...
			Status(...res) { //注意嵌套判断的书写顺序
				...
				res[3].description = res[1] <= new Date().valueOf() && new Date().valueOf() < res[0] ? '抢购中' :
					'即将开抢'
			},
				...
				if (this.countdown != null && e.hours == 0 && e.minutes == 0 && e.seconds == 0) {
					this.times.forEach((item, index) => {
						if (item.spikeStartTimer == this.status.spikeStartTimer) {
							...
							this.timeStamp(StartTimer) <= now && now < this.timeStamp(EndTimer) && (item
								.description == '抢购中', this.countdown = this.timeStamp(EndTimer) - now)
						}
					})
				}
			},
